...
public class LightningBolt extends Entity implements Renderable {
	...
	static {
		...
		imageFrame.add(new Image(ClassResourceUtility.getResourcePath("img/model/LightningBolt/1.png"),
				App.SCREEN_WIDTH, App.SCREEN_HEIGHT, true, false));
		imageFrame.add(new Image(ClassResourceUtility.getResourcePath("img/model/LightningBolt/2.png"),
				App.SCREEN_WIDTH, App.SCREEN_HEIGHT, true, false));
	...
	public LightningBolt(int damage) {
		...
		GameMap.getEntityObjects().parallelStream().filter(x -> x instanceof Hero)
				.forEach(e -> ((Hero) e).decreaseHp(damage));
		lightningSound.play();
	}
